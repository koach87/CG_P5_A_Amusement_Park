...
float IsEdge(in vec2 coords)
{
  ...
  return clamp(0.2*delta,0.0,1.0);
}
void main()
{   
    ...
    vec3 colorOrg = vec3(texture(u_texture, f_in.texture_coordinate));
    vec3 vHSV =  RGBtoHSV(colorOrg.r,colorOrg.g,colorOrg.b);
    vHSV.x = nearestLevel(vHSV.x, 0);
    vHSV.y = nearestLevel(vHSV.y, 1);
    vHSV.z = nearestLevel(vHSV.z, 2);
    float edg = IsEdge(f_in.texture_coordinate);
    vec3 vRGB = (edg >= 5.0)? vec3(0.0,0.0,0.0):HSVtoRGB(vHSV.x,vHSV.y,vHSV.z);
    ...
    // {
    //     vec3 colorOrg = texture(u_texture, uv).rgb;
    //     vec3 vHSV =  RGBtoHSV(colorOrg.r,colorOrg.g,colorOrg.b);
    //     vHSV.x = nearestLevel(vHSV.x, 0);
    //     vHSV.y = nearestLevel(vHSV.y, 1);
    //     vHSV.z = nearestLevel(vHSV.z, 2);
